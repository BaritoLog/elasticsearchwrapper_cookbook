...
refresh_intervals = node['elasticsearch']['index_refresh_intervals']
...
http_request 'Change number_of_replicas in existing index' do
  ...
  action :put
  headers "Content-Type" => "application/json"
  message "{ \"number_of_shards\": \"#{number_of_shards}\", \"number_of_replicas\": \"#{number_of_replicas}\", \"refresh_intervals\": \"#{refresh_intervals}\" }"
  retry_delay 30
  ignore_failure true
end
